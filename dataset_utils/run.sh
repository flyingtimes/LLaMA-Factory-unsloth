...
case "$1" in
    ...
    train)
        ...
        cat  output.jsonl output.jsonl selfreconition.jsonl output.jsonl output.jsonl output.jsonl output.jsonl > data/train.jsonl
        mlx_lm.lora --model Qwen/Qwen1.5-32B-Chat --train --iter 3000 --data ./data --adapter-path ./adapters --max-seq-length 8192 --batch-size 4 --lora-layers 12 
    ...
    retrain)
        ...
        cat  output.jsonl > data/train.jsonl
        mlx_lm.lora --model Qwen/Qwen1.5-32B-Chat --train --iter 3600 --data ./data --resume-adapter-file ./adapters/adapters.safetensors --max-seq-length 8192 --batch-size 4 > trainning.log
    ...
    chat)
        echo "chatting..."
        ../../llama.cpp/main -m ../../$2/ggml-model-q4_0.gguf -c 512 -b 1024 -n 256 --keep 48  --repeat-penalty 1.0 --color -i  -r "<|im_start|>User:" -f ../../llama.cpp/prompts/chat-with-qwen.txt
        ;;
    copy)
